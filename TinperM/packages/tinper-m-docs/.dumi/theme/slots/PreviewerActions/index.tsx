...
const PreviewerActions: FC<IPreviewerActionsProps> = (props) => {
  ...
  const files = Object.entries(props.asset.dependencies).filter(
    ([filePath, { type }]) => (type === 'FILE' && !filePath.includes('/')),
  );
  ...
  const lang = (files[activeKey] && files[activeKey][0].match(/\.([^.]+)$/)?.[1] || 'text') as any;
    ...
    <>
      ...
      {showCode && (
        <>
          ...
          <SourceCode lang={lang}>{files[activeKey] && files[activeKey][1].value}</SourceCode>
        </>
      )}
    </>
  );

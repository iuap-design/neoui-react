...
const NavbarItem: FC<{ data: ReturnType<typeof useNavData>[0] }> = ({ data, }) => {
  ...
  if (activePath) {
    ...
    const isPathMatched = isActivePath ? pathname === activePath : pathname.startsWith(activePath);
    extraProps = isPathMatched ? { className: 'active' } : {};
  }

  return data.link ? (
    disabled ? <>
      <li>
        <a style={{ color: '#888888', cursor: 'not-allowed' }}>{data.title}</a>
      </li>
    </>
      : <div onClick={() => {
        handleReportFunctionClick({
          function_id: `tm_${data.link?.split('/')[1] ? data.link.split('/')[1].replace('-', '_') : 'about'}`,
          function_name: `TinperM${data.title}`,

...
import PageIndicator from '@components/page-indicator/src'
import { staged } from '@utils/StagedComponents'
import { useIsomorphicLayoutEffect, useGetState } from 'tne-fw-fe/hooks'
import { bound } from '@utils/Bounds'
import { mergeFuncProps } from '@utils/WithFuncProps'
import { SwiperProps, SwiperRef } from './iSwiper'
...
function Swiper(props: SwiperProps, ref: React.Ref<SwiperRef>) {
  const { direction, total, children, indicator, clsPrefix, fieldid } = props

  const classPrefix = `${clsPrefix}-swiper`
  ...
  return () => {
    ...
    const [dragging, setDragging, draggingRef] = useGetState(false)
...
export default WebUI({ defaultProps })(
  forwardRef<SwiperRef, SwiperProps>(
    staged<SwiperProps, SwiperRef>(Swiper)
  )
)

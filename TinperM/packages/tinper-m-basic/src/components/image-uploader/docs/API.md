...
| deleteIcon | 删除按钮的 `icon` 图标 | `ReactNode` | `<CloseCircleFill />` |
| disableUpload | 是否禁用上传按钮 | `boolean` | `false` |
| imageFit | 图片填充模式，同 [Image](/basic-components/image) `fit` 属性 | `'contain' \| 'cover' \| 'fill' \| 'none' \| 'scale-down'` | `cover` |
...
| 属性          | 说明           | 类型                       | 其他   |
| ------------- | -------------- | -------------------------- | ------ |
| nativeElement | 原生输入框节点 | `HTMLInputElement \| null` | - |
...
| 属性             | 说明               | 默认值       | 全局变量 |
| ---------------- | ------------------ | ------------ | -------- |
| --cell-size | 图片和上传按钮的大小（仅在非 columns 模式下有效） | `1.28rem` | `--ynfm-size-cell-imageuploader`  |
| --gap | 间距大小 | `0.16rem` | `--ynfm-spacing-gap-imageuploader`  |
...
| 场景             | 生成规则          |
| --------------- | ---------------- |
| 根元素           | fieldid + `"_image_uploader"`         |
| 上传图片           | fieldid + `"_image_uploader_"` + index + `"_item_image"`         |
| 删除按钮           | fieldid + `"_image_uploader_"` + index + `"_item_cell_delete"`         |
| 上传按钮           | fieldid + `"_image_uploader_input"`         |
...
`columns` 属性依赖 [Grid](/basic-components/grid) 布局，该属性存在时，不支持自定义 `--cell-size` 属性，因为图片和上传按钮的大小是自动计算的。
...
const App = () => {
  ...
  return (
    ...
    >
      <span
        className='mui-image-uploader-cell mui-image-uploader-upload-button'
        onClick={handleUpload}
      >
        <span className='mui-image-uploader-upload-button-icon'>
          <AddOutline />
        </span>
      </span>
    </ImageUploader>

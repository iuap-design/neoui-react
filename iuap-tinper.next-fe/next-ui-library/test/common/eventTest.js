...
export default function eventsTest({
	...
	describe(title, () => {
		...
		beforeEach(async () => {
			...
			eventSelect = wrapper.find(selector).at(selectorIndex);
			mocEvent.mockClear();
		});

		// it(`should have a ${selector} selector`, () => {
		...
		it(`${title}should call ${propFuncName}`, async () => {
			...
			eventArgs = eventArgs ?? [];
			eventSelect.simulate(eventName, ...eventArgs)
			if (hasTimeout) {
				jest.runAllTimers();
			}
			...
			if (afterTest) {
				await afterTest(mocEvent, wrapper)
			}
		})
	})
}

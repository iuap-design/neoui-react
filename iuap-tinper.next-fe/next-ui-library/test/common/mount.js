...
export const mount = (component, isDebug) => {
    ...
    const reactWrapper = (node1) => {
        ...
        return {
            ...
            hasClass: (cls) => {
                const isHasC = node.className?.includes?.(cls) || node.getAttribute('class')?.includes(cls) 
                return isHasC
            },
            simulate: (eventType, ...options) => {
                ...
                return reactWrapper(node)
            },
            instance: () => {
                return node
            },
            prop: (v) => {
                return (node[v] || node.getAttribute(v)) ?? undefined
            },
            props: () => {
                const obj = {}
    ...
    find = (matcher, container2) => {
        ...
        let nodes = []
        if (container2) {
            if (container2.querySelectorAll(matcher).length) {
                nodes = container2.querySelectorAll(matcher);
            } else if (match(matcher, container2)) {
                nodes = [container2]
            }
        } else {
            nodes = baseElement.querySelectorAll(matcher);
        }
        return {
            // ...obj,
            ...
            text: () => {
                ...
                // }
                return nodes[0]?.innerText || nodes[0]?.textContent || nodes[0]?.innerHTML
            },
            first: () => {
                const node = nodes[0]
            ...
            prop: (v) => {
                return (nodes[0][v] || nodes[0].getAttribute(v)) ?? undefined
            },
            update: () => undefined,
            find: (matcher) => find(matcher, nodes[0]),
            hasClass: (cls) => {
                const isHasC = nodes[0].className.includes?.(cls) || nodes[0].getAttribute('class').includes(cls) 
                return isHasC
            },
            simulate: (eventType, ...options) => {
                fireEvent[eventTypeMap[eventType] || eventType](nodes[0], ...options)
                return reactWrapper(nodes[0])
            },
            forEach: (callBack) => {
...
// export default {mount, shallow: mount, render: mount}
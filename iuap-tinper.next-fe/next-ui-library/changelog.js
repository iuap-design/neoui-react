...
// const conventionalChangelog = require('conventional-changelog-cli');
...
async function changlog(oldId, newId, newTag) {
	...
	// let AuthorReg = /Author: .+<[a-zA-Z]+@yonyou\.com>/g
	...
	try{
		...
		Object.keys(log_obj).forEach(key => {
			...
			// mdJson.push(`* ${CommitType[key.split('_')[0]]}  【${log_obj[key].length}】`)
			handleAllJson2md(log_obj[key], key.split('_')[0])
		});
		// mdJson.push({link: { title: '更新日志', source: `https://design.yonyoucloud.com/static/tinper-next/${newTag}/GIT_CHANGELOG/All_CHANGELOG.md`}});
	} catch(err) {
		console.error(err);
	}
	...
	// fs.writeFileSync('./GIT_CHANGELOG/All_CHANGELOG.md', json2md(all_mdJson), 'utf-8');

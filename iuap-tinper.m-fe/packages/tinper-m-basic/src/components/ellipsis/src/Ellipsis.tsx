...
import { useResizeEffect, useIsomorphicLayoutEffect } from 'tne-fw-fe/hooks'
import { withStopPropagation } from '@utils/WithStopPropagation'
import { EllipsisProps } from './iEllipsis'
...
function Ellipsis(props: EllipsisProps) {
  ...
  const classPrefix = `${clsPrefix}-ellipsis`
      ...
      function check(left: number, right: number): EllipsisedValue {
        if (right - left <= 1) {
          if (props.direction === 'end') {
            return { leading: getSubString(0, left) + '...', }
          } else {
            return { tailing: '...' + getSubString(right, end), }
          }
        }
...
export default WebUI({ defaultProps: defaultProps })(Ellipsis)

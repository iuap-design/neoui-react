...
import { useSize } from 'tne-fw-fe/hooks'
import { rubberbandIfOutOfBounds } from '@utils/Rubberbands'
import { useDragAndPinch } from 'tne-fw-fe/hooks'
import { bound } from '@utils/Bounds'
import type { Matrix } from '@utils/Matrixs'
import * as mat from '@utils/Matrixs'
...
export const Slide: FC<Props> = props => {
  const { dragLockRef, maxZoom, clsPrefix, fieldid } = props
  const _clsPrefix = `${clsPrefix}-slide`
  ...
  const getReachBound = (
    ...
    max: number,
    buffer = 0
  ) => [position <= min - buffer, position >= max + buffer]
  ...
  useDragAndPinch(
    {
      onDrag: state => {
        if (state.first) {
          const { x: { position: x, minX, maxX }, } = getMinAndMax(matrix.get())
        ...
        if (!pinchLockRef.current && currentZoom <= 1) {
          ...
          const { x: { position: x, minX, maxX }, } = getMinAndMax(nextMatrix)
          if (
            state.last &&
  ...
  return (
    <div className={`${_clsPrefix}`} fieldid={fieldid}>
      <div className={`${_clsPrefix}-control`} ref={controlRef}>
        <animated.div
          className={`${_clsPrefix}-image-wrapper`}
          style={{ matrix, }}
        >
          <img
            fieldid={fieldid ? `${fieldid}_img` : undefined}
            ref={imgRef}
            src={props.image}
            draggable={false}
            alt={props.image}

...
async function main() {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    let _path = path.join('static/tinper-m/' + version + '/', fileName.replace('./lib/bundle/', ''))
    form.append('file', fs.createReadStream(fileName));
    form.append('type', 'put');
    form.append('path', _path);
    // PromiseArr.push(client.put('static/tinper-m/' + version + '/' + fileName, fs.createReadStream('./lib/bundle/' + fileName)).catch(err => console.log(err)))
    PromiseArr.push(fetch('', { method: 'post', body: form })
      .then(res => res.json()))
    if (!feature && !betaFix) { // feature分支不上传固定的tag目录
      const form = new formData();
      const fileName = arr[i];
      _path = path.join('static/tinper-m/' + tag + '/', fileName.replace('./lib/bundle/', ''))
      fileList.push(_path)
      form.append('file', fs.createReadStream(fileName));
      form.append('type', 'put');
      form.append('path', _path);
  ...
  const refreshUrl = [
    'https://design.yonyoucloud.com/static/tinper-m/' + tag + '/tinper-m.min.js',
    'https://design.yonyoucloud.com/static/tinper-m/' + tag + '/tinper-m.css'
  ];
  let j = 0;
  for (let i = 0; i < refreshUrl.length; i++) {
    ...
    const res = await axios({
      method: 'post',
      data: { url: encodeURIComponent(item) },
      url: ''
    })
    if (res.status === 200) {

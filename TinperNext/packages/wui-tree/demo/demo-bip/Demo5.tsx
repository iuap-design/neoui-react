...
class Demo5 extends Component<{}, {treeData:GlobalDataType, loadedKeys: string[]}> {
    constructor(props: {}) {
        super(props);
        this.state = {
            treeData: [],
    ...
    onLoad: TreeProps['onLoad'] = (loadedKeys, ...args) => {
        this.setState({
            loadedKeys
        })
        console.log(loadedKeys, ...args)
    }
    render() {
        const {loadedKeys, treeData} = this.state;
        ...
        return (
            <Tree className="myCls" loadedKeys={loadedKeys} defaultExpandedKeys={['0-1']} onLoad={this.onLoad} onSelect={this.onSelect} loadData={this.onLoadData}>
                {treeNodes}
            </Tree>
        );
    }

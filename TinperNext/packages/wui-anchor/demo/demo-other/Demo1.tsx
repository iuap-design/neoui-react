 ...
 *
 * @title 容器内示例
 * @description 在固定高度容器内的示例
 *
 */
import {Anchor} from '@tinper/next-ui'
import React, {Component} from 'react';

// let {Link} = Anchor

class Demo1 extends Component<{}, {anthors?: {href: string, name: string, content: string}[]}> {
    constructor(props: {}) {
        super(props);
        this.state = {
            anthors: [
    ...
    componentDidMount() {
        if (document.querySelectorAll('.demo-example')[1]) {
            (document.querySelectorAll('.demo-example')[1] as HTMLElement).style.position = 'relative'
        }
        // (document.querySelectorAll('.demo-example')[1] as HTMLElement).style.position = 'relative'
    }
	handlegetContainer = () => {
	...
	render() {
	    ...
	    return (
	        <div className="demo1" id="container-box" style={{height: '300px', overflowY: 'auto'}}>
	            <Anchor antd offsetTop={150} style={{width: '10%', marginLeft: '90%'}} getContainer={this.handlegetContainer as ()=> HTMLElement}
	                affix={false} items={items}>
	                {/* {

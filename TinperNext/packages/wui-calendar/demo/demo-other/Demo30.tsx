...
class Demo30 extends Component <{}, DemoState> {
    ...
    onMoreEvent = (_e: any, val: {startTitleValue: string, titleValue: string, tex: string}[], date: string) => {
        // console.log('点击更多回调', _e, val, new Date(date).getDay())
        let { type } = this.state
        const days = ["周日", "周一", "周二", "周三", "周四", "周五", "周六"];
        let weekNum = days[new Date(date).getDay()]
        if (type == 'date') {
            weekNum = days[new Date(date).getDay()]
        } else if (type == 'month') {
            weekNum = (new Date(date).getMonth() + 1) + '月'
        }
        let domNode = (<div style={{pointerEvents: 'none'}}>
            <div style={{display: 'flex', justifyContent: 'space-between', padding: '5px'}}>
                <span><span style={{fontWeight: '600', fontSize: "20px", paddingLeft: '20px', marginRight: '5px'}}>{date.split('-')[2]}</span><span>{weekNum}</span></span>
                <span onClick={this.onClose} style={{pointerEvents: 'all'}}><Icon type="uf-close" style={{fontSize: '12px'}} /></span>
            </div>
            <div style={{padding: '5px 10px', maxHeight: '110px', overflowY: 'auto', scrollbarWidth: 'thin'}}>
                {
    ...
    onCellClick = (_e: any, val: {startTitleValue: string, titleValue: string, tex: string}, date: string) => {
        ...
        let domNode = (<div style={{pointerEvents: 'none'}}>
            <div style={{display: 'flex', justifyContent: 'flex-end', background: '#aaa', padding: '5px 5px 0', borderTopLeftRadius: '4px', borderTopRightRadius: '4px'}}>
                <span title="自定义功能"><Icon type="uf-tuichudenglu" /></span>
                <span onClick={this.onClose} style={{pointerEvents: 'all'}}><Icon type="uf-close" style={{fontSize: '14px'}} /></span>
            </div>
            <div style={{background: '#aaa', padding: '5px'}}>
                <span style={{fontWeight: 'bold', paddingLeft: '5px', paddingBottom: '10px', fontSize: "16px", display: 'inline-block', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', maxWidth: '300px'}} title={val.tex}>{val.tex}</span>
            </div>
